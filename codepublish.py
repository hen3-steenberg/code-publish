...
formatter = LatexFormatter(linenos = True, texcomments = False, mathescape = True)
...
skip = [".vs", "out", "__pycache__", "publish", "CMakeLists.txt", skip_file_name, publish_file_name]
...
def get_folder_skip(dir: str) -> list[str]:
    if exists(f'{dir}/{skip_file_name}'):
        ...
        result = sfile.readlines()
        sfile.close()
        return result
    else: return []

def get_folder_items_auto(dir: str, extensions: list[str], include_subfolders: bool = True) -> list[str]:
    content = []
...
def get_folder_items(dir: str, extensions: list[str], include_subfolders: bool = True) -> list[str]:
...
def publish_assignment(dir: str, title:str, subtitle:str, author:str) -> tuple[str, str]:
...
def publish_assignment_to_pdf(dir: str, title:str, subtitle:str, author:str) -> tuple[str, str]:
    file, pub_dir = publish_assignment(dir, title, subtitle, author)
    wd = getcwd()
    chdir(pub_dir)
    subprocess.run(["lualatex", "--shell-escape", f'"{file}"'])
    chdir(wd)
...
if __name__ == "__main__":
    file, dir = publish_assignment_to_pdf("testassignment","Test Assignment", "C++", "Name Surname (12345678)")
    print(f'{dir}/{file}')
